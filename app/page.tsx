...
const navigation = [
  { label: 'Work', linkType: 'page' as const, target: { slug: 'projects' } },
  { label: 'About', linkType: 'page' as const, target: { slug: 'about' } },
  { label: 'Contact', linkType: 'page' as const, target: { slug: 'contact' } },
]
        ...
        <div className="max-w-6xl mx-auto px-6 py-24 w-full">
          <div className="max-w-2xl animate-deliberate">
            ...
            <p className="text-lg md:text-xl text-ink/70 leading-relaxed mb-8 max-w-reading">
              Designer and developer crafting digital experiences at the intersection of form and function. Currently available for select projects.
            </p>
            <Link href="/projects" className="inline-flex items-center justify-center px-6 py-3 bg-ember text-stone font-body font-medium text-sm rounded-ledger transition-all duration-100 hover:brightness-110">
              View work
            </Link>
          </div>
        </div>
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.techStack.slice(0, 3).map((tag) => (
                      <span key={tag} className="inline-flex items-center px-3 py-1 text-xs font-mono font-medium tracking-wider uppercase border border-moss text-moss rounded-ledger">
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <div className="max-w-6xl mx-auto px-6 py-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div>
              ...
              <p className="text-stone/70 text-sm leading-relaxed">Designer and developer crafting digital experiences.</p>
            </div>
            
            <nav>
              <h4 className="text-xs font-mono font-medium tracking-wider uppercase text-brass mb-4">Navigation</h4>
              <ul className="space-y-2">
                {navigation.map((item) => (
                  <li key={item.label}>
                    <Link href={`/${item.target?.slug || ''}`} className="text-sm text-stone/70 hover:text-stone transition-colors duration-100">
                      {item.label}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              <h4 className="text-xs font-mono font-medium tracking-wider uppercase text-brass mb-4">Connect</h4>
              <div className="flex gap-4">
                <a href="#" className="text-stone/70 hover:text-stone transition-colors duration-100" aria-label="GitHub">
                ...
                <a href="#" className="text-stone/70 hover:text-stone transition-colors duration-100" aria-label="Twitter">
